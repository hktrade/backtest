...
def get_two(): # run From Monday 21:00 to Friday 21:00
	...
	print(US_15,'\t -1 ',US_today)
	
	print(US_before, US_last, US_15)
	# print(early['market_close'].iloc[-1][0:19])#2022-05-06 20:00:00
	# mc = int(time.mktime(early['market_close'].iloc[-1][0:19]))
	print(type(early['market_close'].iloc[-1]))
	return(early['market_close'].iloc[-1])
...
# start = pd.Timestamp('2010-01-01', tz='America/New_York').isoformat()
	...
	if True: 
		...
		if len(df_api)==0:
			return None
		print(symbols, N_min, len(df_api))
